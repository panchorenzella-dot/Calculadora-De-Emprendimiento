...
const availableSections = calculatorSections.map((section) => ({
  ...section,
  calculators: section.calculators.filter((calculator) => !calculator.comingSoon),
}));
const comingSoonCalculators = calculatorSections.flatMap((section) =>
  section.calculators.filter((calculator) => calculator.comingSoon),
);
const featuredCalculator = availableCalculators.find(
  (calculator) => calculator.href === "/margen",
)!;
          ...
          <div className="relative mt-4 grid gap-6">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-white/60 sm:text-base">
                Elegí tu tipo de negocio o explorá las herramientas de inversión,
                rentabilidad e impuestos para tu próxima decisión.
              </p>
            </div>
          </div>
        </section>

        <section aria-labelledby="featured-calculator-title" className="mt-8">
          <Link
            href={featuredCalculator.href}
            onClick={() =>
              trackEvent("select_calculator", {
                calculator_name: featuredCalculator.title,
                calculator_category: "business",
                destination: featuredCalculator.href,
                source: "calculator_catalog_featured",
              })
            }
            className="group flex flex-col gap-6 rounded-3xl border border-emerald-300/30 bg-gradient-to-br from-emerald-300/10 to-sky-300/[0.04] p-6 transition hover:border-emerald-300/60 hover:bg-emerald-300/10 sm:flex-row sm:items-center sm:justify-between sm:p-8"
          >
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-200">
                Calculadora destacada
              </p>
              <h2 id="featured-calculator-title" className="mt-3 text-2xl font-bold tracking-tight sm:text-3xl">
                {featuredCalculator.title}
              </h2>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-white/70">
                {featuredCalculator.description}
              </p>
            </div>
            <span className="shrink-0 rounded-full bg-emerald-200 px-5 py-3 text-center text-sm font-bold text-zinc-950 transition group-hover:bg-emerald-100">
              Calcular mi margen <span aria-hidden="true">→</span>
            </span>
          </Link>
        </section>
        ...
        <div id="calculator-results">
          <div className="mt-12 space-y-14">
            {availableSections.map((section) => {
              ...
              return (
                <section key={section.id} id={section.id} aria-labelledby={sectionTitleId}>
                  <div className="mb-6">
