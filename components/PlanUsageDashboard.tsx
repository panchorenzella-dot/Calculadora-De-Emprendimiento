  ...
  return (
    <article className="rounded-3xl border border-white/[0.08] bg-black/20 p-5">
      ...
      <div className="mt-5 flex items-center justify-between border-t border-white/[0.06] pt-4 text-xs">
        ...
        {item.resets_at && !unlimited && <span className="text-emerald-100/55">Renueva {date(item.resets_at)}</span>}
      </div>
    </article>
  );
}
...
export default function PlanUsageDashboard({ plan, usage }: { plan: PlanPeriod; usage: UsageItem[] }) {
  ...
  const period = useMemo(() => {
    if (plan.plan !== "pro" || !plan.current_period_start || !plan.current_period_end) return null;
    const start = new Date(plan.current_period_start).getTime();
    ...
    return { progress, daysLeft, end: plan.current_period_end, interval };
  }, [now, plan]);
    ...
    <>
      ...
      {period ? (
        <section className="mt-8 rounded-3xl border border-emerald-300/15 bg-[linear-gradient(135deg,rgba(16,185,129,0.09),rgba(255,255,255,0.02))] p-6 sm:p-7">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-200/55">Próximo pago</p><h2 className="mt-2 text-2xl font-semibold tracking-tight">{date(period.end)}</h2><p className="mt-2 text-sm text-white/40">Ciclo {period.interval} · {period.daysLeft} {period.daysLeft === 1 ? "día restante" : "días restantes"}</p></div><div className="rounded-full border border-white/10 bg-black/20 px-3 py-1.5 text-xs text-white/45">{plan.cancel_at_period_end ? "Finaliza ese día" : "Renovación automática"}</div></div>
          <div className="mt-7 h-2 overflow-hidden rounded-full bg-white/[0.07]"><div className="h-full rounded-full bg-gradient-to-r from-emerald-500 to-emerald-200 transition-[width] duration-700" style={{ width: `${period.progress}%` }} /></div>
          <div className="mt-3 flex justify-between text-[11px] text-white/28"><span>Inicio del ciclo</span><span>{Math.round(period.progress)}% transcurrido</span><span>Fecha de pago</span></div>
