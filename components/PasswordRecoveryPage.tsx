...
export default function PasswordRecoveryPage() {
  ...
  return (
    <main className="calculator-theme min-h-[72vh] px-4 py-10 text-white sm:py-16">
      <section className="mx-auto max-w-lg overflow-hidden rounded-[28px] border border-zinc-200 bg-white p-6 text-zinc-950 shadow-[0_28px_90px_rgba(0,0,0,.5)] sm:p-9">
        <span className="grid h-11 w-11 place-items-center rounded-2xl bg-zinc-950 text-sm font-black text-white">CE</span>
        <p className="mt-7 text-xs font-black uppercase tracking-[0.18em] text-green-700">Seguridad de la cuenta</p>
        <h1 className="mt-2 text-3xl font-black tracking-[-0.035em]">Creá una contraseña nueva</h1>

        {checking ? (
          <p className="mt-6 text-sm font-semibold text-zinc-500">Verificando tu enlace seguro...</p>
        ) : success ? (
          <div className="mt-7">
            <p className="rounded-xl border border-green-200 bg-green-50 p-4 text-sm font-semibold leading-6 text-green-800">Tu contraseña se actualizó correctamente.</p>
            <Link href="/perfil" className="mt-5 flex min-h-12 items-center justify-center rounded-xl bg-green-700 px-4 py-3 text-sm font-black text-white hover:bg-green-800">Ir a mi cuenta</Link>
          </div>
        ) : ready ? (
          <form onSubmit={updatePassword} className="mt-7 grid gap-4">
            <label className="grid gap-2 text-xs font-bold text-zinc-800">
              Nueva contraseña
            ...
            <button disabled={saving} className="mt-1 min-h-12 rounded-xl bg-green-700 px-4 py-3 text-sm font-black text-white hover:bg-green-800 disabled:opacity-55">{saving ? "Guardando..." : "Guardar nueva contraseña"}</button>
          </form>
        ) : (
          <div className="mt-6">
            ...
            <Link href="/perfil" className="mt-5 inline-flex text-sm font-black text-green-700 hover:text-green-900">Volver a iniciar sesión →</Link>
          </div>
        )}
      </section>
    </main>
